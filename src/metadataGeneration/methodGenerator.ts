...
export class MethodGenerator {
  ...
  public Generate(): Tsoa.Method {
    if (!this.IsValid()) {
      throw new GenerateMetadataError(
        "This isn't a valid a controller method."
      );
    }
  ...
  private processMethodDecorators() {
    const pathDecorators = getDecorators(this.node, identifier =>
      this.supportsPathMethod(identifier.text)
    );
    ...
    if (pathDecorators.length > 1) {
      throw new GenerateMetadataError(
        ...
        }' method, Found: ${pathDecorators.map(d => d.text).join(', ')}`
      );
    }
  ...
  private getMethodResponses(): Tsoa.Response[] {
    ...
    return decorators.map(decorator => {
      ...
      if (
        ...
        (expression.arguments[1] as any).text
      ) {
  ...
  private getMethodSuccessResponse(type: Tsoa.Type): Tsoa.Response {
    const decorators = this.getDecoratorsByIdentifier(
      this.node,
      'SuccessResponse'
    );
    if (!decorators || !decorators.length) {
    ...
    if (decorators.length > 1) {
      throw new GenerateMetadataError(
        `Only one SuccessResponse decorator allowed in '${
          this.getCurrentLocation
        }' method.`
      );
    }
  ...
  private getMethodSuccessExamples() {
    const exampleDecorators = this.getDecoratorsByIdentifier(
      this.node,
      'Example'
    );
  ...
  private supportsPathMethod(method: string) {
    return ['get', 'post', 'put', 'patch', 'delete', 'head'].some(
  ...
  private getOperationId() {
    const opDecorators = this.getDecoratorsByIdentifier(
      this.node,
      'OperationId'
    );
    ...
    if (opDecorators.length > 1) {
      throw new GenerateMetadataError(
        `Only one OperationId decorator allowed in '${
          this.getCurrentLocation
        }' method.`
      );
    }
  ...
  private getTags() {
    ...
    if (tagsDecorators.length > 1) {
      throw new GenerateMetadataError(
        `Only one Tags decorator allowed in '${
          this.getCurrentLocation
        }' method.`
      );
    }
  ...
  private getIsHidden() {
    const hiddenDecorators = this.getDecoratorsByIdentifier(
      this.node,
      'Hidden'
    );
    ...
    if (hiddenDecorators.length > 1) {
      throw new GenerateMetadataError(
        `Only one Hidden decorator allowed in '${
          this.getCurrentLocation
        }' method.`
      );
    }
  ...
  private getSecurity(): Tsoa.Security[] {
    const securityDecorators = this.getDecoratorsByIdentifier(
      this.node,
      'Security'
    );

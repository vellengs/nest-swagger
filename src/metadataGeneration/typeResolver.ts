...
export class TypeResolver {
  ...
  public resolve(): Tsoa.Type {
    ...
    if (this.typeNode.kind !== ts.SyntaxKind.TypeReference) {
      throw new GenerateMetadataError(
        `Unknown type: ${ts.SyntaxKind[this.typeNode.kind]}`
      );
    }
    ...
    if (!this.extractEnum) {
      const enumType = this.getEnumerateType(
  ...
  private getReferenceType(
    ...
    try {
      ...
      const referenceEnumType = this.getEnumerateType(
        type,
        true
      ) as Tsoa.ReferenceType;
      ...
      const inheritedProperties =
        this.getModelInheritedProperties(modelType) || [];
  ...
  private getAnyTypeName(typeNode: ts.TypeNode): string {
    ...
    if (typeNode.kind !== ts.SyntaxKind.TypeReference) {
      throw new GenerateMetadataError(
        `Unknown type: ${ts.SyntaxKind[typeNode.kind]}.`
      );
    }
  ...
  private resolveModelTypeScope(
    ...
    while (
      leftmost.parent &&
      leftmost.parent.kind === ts.SyntaxKind.QualifiedName
    ) {
      const leftmostName =
      ...
      const moduleDeclarations = statements.filter(node => {
        ...
        return (
          (moduleDeclaration.name as ts.Identifier).text.toLowerCase() ===
      ...
      if (
        moduleBlock === null ||
        moduleBlock.kind !== ts.SyntaxKind.ModuleBlock
      ) {
        throw new GenerateMetadataError(
    ...
    if (modelTypes.length > 1) {
      ...
      const designatedModels = modelTypes.filter(modelType => {
        const isDesignatedModel = isExistJSDocTag(
          modelType,
          tag => tag.tagName.text === 'tsoaModel'
        );
  ...
  private getModelProperties(
      ...
      if (aliasDeclaration.type.kind === ts.SyntaxKind.IntersectionType) {
        ...
        intersectionTypeNode.types.forEach(type => {
          if (type.kind === ts.SyntaxKind.TypeReference) {
            ...
            const modelType = this.getModelTypeDeclaration(
    ...
    if (classConstructor && classConstructor.parameters) {
      const constructorProperties = classConstructor.parameters.filter(
        parameter => this.hasPublicModifier(parameter)
      );
  ...
  private getNodeDescription(
    ...
    const symbol = this.current.typeChecker.getSymbolAtLocation(

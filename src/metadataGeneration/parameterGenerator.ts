...
export class ParameterGenerator {
  ...
  private getHeaderParameter(
    ...
    if (!this.supportPathDataType(type)) {
      throw new GenerateMetadataError(
        `@Header('${parameterName}') Can't support '${type.dataType}' type.`
      );
    }
  ...
  private getQueryParameter(
    ...
    if (type.dataType === 'array') {
      ...
      if (!this.supportPathDataType(arrayType.elementType)) {
        throw new GenerateMetadataError(
          `@Query('${parameterName}') Can't support array '${
    ...
    if (!this.supportPathDataType(type)) {
      throw new GenerateMetadataError(
        `@Query('${parameterName}') Can't support '${type.dataType}' type.`
      );
    }
  ...
  private getPathParameter(parameter: ts.ParameterDeclaration): Tsoa.Parameter {
    ...
    const type = this.getValidatedType(parameter, false);
    const pathName =
      getDecoratorTextValue(this.parameter, ident => ident.text === 'Path') ||
      parameterName;

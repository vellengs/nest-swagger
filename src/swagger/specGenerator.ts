...
export class SpecGenerator {
  constructor(
    private readonly metadata: Tsoa.Metadata,
  ...
  public GetSpec() {
    let spec: Swagger.Spec = {
      basePath: normalizePath(
        this.config.basePath as string,
        '/',
        undefined,
        false
      ),
  ...
  private buildPaths() {
    ...
    this.metadata.controllers.forEach(controller => {
      const normalisedControllerPath = normalizePath(controller.path, '/');
      // construct documentation using all methods except @Hidden
      controller.methods
        .filter(method => !method.isHidden)
        .forEach(method => {
          const normalisedMethodPath = normalizePath(method.path, '/');
          const path = normalizePath(
            `${normalisedControllerPath}${normalisedMethodPath}`,
            '/',
            '',
            false
          );
  ...
  private buildMethod(
    ...
    const pathMethod: Swagger.Operation = (pathObject[
      method.method
    ] = this.buildOperation(controllerName, method));
    ...
    const bodyPropParameter = this.buildBodyPropParameter(
      controllerName,
      method
    );
    ...
    source.forEach(property => {
      ...
      if (!swaggerType.$ref) {
        ...
        Object.keys(property.validators)
          .filter(key => {
            return (
              !key.startsWith('is') && key !== 'minDate' && key !== 'maxDate'
            );
          })
          .forEach(key => {
  ...
  private buildOperation(
    ...
    method.responses.forEach((res: Tsoa.Response) => {
      ...
      if (res.examples) {
        swaggerResponses[res.name].examples = {
  ...
  private getSwaggerType(type: Tsoa.Type): Swagger.Schema {
    ...
    return this.getSwaggerTypeForReferenceType(
      type as Tsoa.ReferenceType
    ) as Swagger.Schema;
  }
  ...
  private getSwaggerTypeForArrayType(
    arrayType: Tsoa.ArrayType
  ): Swagger.Schema {
    return { type: 'array', items: this.getSwaggerType(arrayType.elementType) };
  }

  private getSwaggerTypeForEnumType(
    enumType: Tsoa.EnumerateType
  ): Swagger.Schema {
    return {
      type: 'string',
      enum: enumType.enums.map(member => String(member))
    };
  }
